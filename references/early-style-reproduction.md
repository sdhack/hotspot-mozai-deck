...
默认将每页标题、正文、标签、页码以及（如页面需要）互动文字直接生成在图上，并逐字核对。为保证可读性，把长正文拆成短句、信息卡或清单；标题通常控制在两行内，但不能为了少字而漏掉既定页面文案。文字错误经过两次定向重试仍存在时，说明限制并等待用户决定是否切换为后期排字，不能以模糊文字交付，也不能擅自改成少字版本。
...
顶部显示 `04/04`，一行安抚但不轻率的结论标题。保留大面积空白，将一条经核验的安全提醒或（仅在用户要求时）具体互动问题放在可后期排版的对话框中。墨仔居中或下方举牌 / 挥手；背景使用与主题相关的简洁象征物，避免新增事实。
